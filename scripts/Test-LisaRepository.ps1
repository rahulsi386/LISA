...
try {
    $SkillsRoot = Join-Path $RepositoryRoot 'github-copilot-cli/skills'
    if ($Suite -eq 'Plugin') {
        Invoke-RepositoryCheck python @('-m', 'unittest', 'discover', '-s', 'github-copilot-cli/tests', '-p', 'test_*.py', '-q')
        ...
        Invoke-RepositoryCheck node @('github-copilot-cli/skills/artifact-publisher/tests/test_fast_publisher.js')
        Invoke-RepositoryCheck npm @('--prefix', 'github-copilot-cli/skills/solution-designer/renderer', 'test')
    }
    elseif ($Suite -eq 'Scout') {
        ...
        Invoke-RepositoryCheck python @('github-copilot-cli/skills/agent-optimizer/tests/test_gepa.py', '-v')
    }
