...
7. Never infer consent from “yes”, prior consent, or another operation. Require the exact phrase `DELETE OUTPUT` through the host's free-text user-question interaction and stop while waiting. If no question tool is available, ask in chat and end the turn.
...
3. Ask the user to type `DELETE OUTPUT` using the host's free-text user-question interaction. Stop and wait.
