    ...
    <section id="${diagram.id}" aria-labelledby="${diagram.id}-title">
      <div class="section-heading">
        ...
        <div class="downloads" aria-label="${diagram.title} files">
          ...
          <a href="${file}.png" download>Download PNG</a>
        </div>
      </div>
      ...
      <div class="viewport" tabindex="0" role="region" aria-label="${diagram.title} image">
        <img src="${file}.png" alt="${esc(model.title)} - ${diagram.title}" decoding="async">
      </div>
    </section>`;
  }).join("\n");
  return `<!doctype html>
...
<body>
  ...
  <footer>Offline preview. Keep this file beside its four SVG and PNG diagram files when moving or sharing the artifact folder.
