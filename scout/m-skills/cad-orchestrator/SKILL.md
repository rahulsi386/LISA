...
2. Otherwise the project default: `<scout-data-dir>\LISA\lisa-config.json`.
...
```powershell
python "<local-skills-root>\lisa_path_resolver.py" --config "<CONFIG>"
```
...
```powershell
python "<local-skills-root>\workflow_checkpoint.py" --config "<CONFIG>" init
python "<local-skills-root>\workflow_checkpoint.py" --config "<CONFIG>" recover
```
...
- Invoke each stage skill **directly in this session**. Do not delegate a whole skill to a nested
  task or subagent.
- Load the skill with `m_get_skill` before invoking it, and follow its instructions.
...
- Follow `<local-skills-root>\workflow-checkpointing.md` at every phase boundary. A stage starts
...
infer approval from silence, earlier instructions, or unrelated consent. At each prompt, call
`m_ask_user` and stop immediately after the tool call. Resume only from the user's next turn.
...
2. Persist the pause:
   ...
   ```powershell
   python "<local-skills-root>\workflow_checkpoint.py" --config "<CONFIG>" checkpoint --phase "human-review" --unit-id "classification-decision" --status WAITING
   ```

3. Call `m_ask_user` with:
...
2. Persist the pause:
   ...
   ```powershell
   python "<local-skills-root>\workflow_checkpoint.py" --config "<CONFIG>" checkpoint --phase "human-review" --unit-id "build-decision" --status WAITING
   ```
...
6. On **Revise**, checkpoint `WAITING` with unit `build-revision-feedback`, then call `m_ask_user`
   without `answers`:
...
```powershell
python "<local-skills-root>\workflow_checkpoint.py" --config "<CONFIG>" finish --status COMPLETED
```
