# LISA on Microsoft Scout Desktop
...
The installer resolves `requirements.txt` relative to its own root location, validates
the optional `scout\m-automations` folder, and deploys only the contents of `scout\m-skills`
into Scout.
...
It asks which platform to target; choose **Microsoft Scout Desktop** for this guide.
Run it from the downloaded repository root. Existing project data is preserved:

```powershell
Set-Location "<downloaded-LISA-codebase>"

# Read-only prerequisite assessment; exits before sign-in or file changes.
powershell.exe -ExecutionPolicy Bypass -File .\Install-LISA-Prerequisites.ps1 -Platform Scout -WhatIf

# Interactive installation.
powershell.exe -ExecutionPolicy Bypass -File .\Install-LISA-Prerequisites.ps1
```

Use `-ProjectPath "C:\Projects\MySolution"` for a custom project location, or `-SkipProjectSetup`
to install only the Scout skills. Installation initializes missing registry entries automatically.
...
1. Checks WinGet, Microsoft Scout, Windows PowerShell 5.1, PowerShell 7, Python, the Python
  libraries in the sibling root `requirements.txt`, Node.js 20+/npm/npx, the renderer and Python layout
  router beneath `scout\m-skills`, modern Power Platform CLI support, and conditional .NET 10 availability.
...
5. Prompts for the downloaded LISA codebase root. Supply the same root shown above, not its
  `scout\m-skills` child. The installer validates a non-empty `scout\m-skills`, optional non-empty
  `scout\m-automations`, and valid root `lisa-config.json`.
6. Warns that matching entries in `%USERPROFILE%\.scout\m-skills` will be replaced and waits for
  the exact confirmation `INSTALL SKILLS`.
7. Stages and installs the skills beneath `scout\m-skills`, retaining unrelated installed skills
  and the existing metadata registry.
8. Runs `sync_skills_metadata.py --initialize --skills ...` for the installed LISA skills, creating
  missing entries while preserving existing IDs, enabled settings, and unrelated metadata.
9. Creates missing project folders and configuration, unless `-SkipProjectSetup` is specified.
  Existing configuration, requirements, evaluation data, output, and checkpoints remain unchanged.
10. Asks the user to restart Scout so it reloads the registry.

The installer does not copy `scout\m-automations`; it validates them as distribution inputs.
It copies the common `lisa-config.json` only when the project has no configuration. It does not
authenticate PAC, provision cloud licenses or capacity,
...
> [!CAUTION]
> Installing skills replaces matching skill implementations after confirmation. Back up local
> customizations before upgrading. Project data and existing configuration are not replaced.
...
software and skills, synchronizes the skill registry, and creates any missing project files under:
...
> [!CAUTION]
> Keep `basePath` as `.` when the config is in the project root. Paths resolve relative to the
> configuration file. Environment tokens such as `%USERPROFILE%` are not expanded inside JSON.
>
...
| Node.js, npm and npx | Node.js 20+; 24.13.0 verified | Installs `OpenJS.NodeJS.LTS` when missing or too old |
...
Existing project contents are allowed and preserved. The installer rejects files in place of
required directories and symlink/junction paths. Choose a regular project directory with
`-ProjectPath`, or use `-SkipProjectSetup` to install skills without initializing a project.

### The downloaded codebase is rejected

Select the codebase root, not its `scout\m-skills` child. The selected root must contain a non-empty
`scout\m-skills`, a valid `lisa-config.json`, and optionally a non-empty `scout\m-automations`. The source
`scout\m-skills` must contain child `SKILL.md` files and `sync_skills_metadata.py`.
The installer creates missing registry entries in the installed Scout directory; no generated
registry is required in the source checkout.
...
Normal synchronization still rejects unknown skills. During installation, `--initialize` creates
missing local entries and `--skills` limits updates to the installed LISA skill directories.
Existing IDs, enabled settings, and unrelated entries are preserved. Invalid registries are
rejected rather than reset. Keep credentials out of definitions and generated metadata.

Shared implementation and contract files under `scout\m-skills` include:
